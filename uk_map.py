def uk_map(fig1, indata, clevs, datlons, datlats, mtitle, munits, maskswitch):
	"""potentially excessively complicated but very pretty code that makes a topography 
	inclusive map of the UK within a figure positon ('fig1'), given a dataset on an x*y grid 
	('indata'), lon and lat arrays on the same grid ('datlons', 'datlats'), contour levels 
	('clevs'), a title ('mtitle'), and the correct units ('mtitle', 'munits'). 
	Includes a configurable scale for colour coding contours to any given function.
	
	Can be asked to mask out oceans ('maskswitch'), but if so needs a mask-map 'miss.nc'
	"""
